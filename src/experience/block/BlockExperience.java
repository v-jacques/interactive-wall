...
public class BlockExperience extends Listener implements Experience {
	...
	public BlockExperience() {
		...
		Image backImg = new Image("media/block_background1600_900.jpg", 1600,
				1000, true, true);
		...
		Image exitImageHovered = new Image("media/ExitHovered180_180.png", 150,
				150, true, true);

		Image addSquareBlockNormal = new Image("media/BlockPlus60_60.png", 125,
				125, true, true);
		addSquareBlockImage = new ImageView(addSquareBlockNormal);

		Image addSquareBlockHovered = new Image("media/BlockPlusHovered.png",
				125, 125, true, true);

		Image addRectBlockNormal = new Image("media/BlockPlus60_60.png", 125,
				125, true, true);
		addRectBlockImage = new ImageView(addSquareBlockNormal);

		Image addRectBlockHovered = new Image("media/BlockPlusHovered.png",
				125, 125, true, true);

		palmRightNormal = new Image("media/palmRight.png", 100, 100, true, true);

		rightHand = new ImageView(palmRightNormal);
		Image rightHandFull = new Image("media/Hold_fullHand_102_107.png", 100,
				100, true, true);

		pointNormal = new Image("media/tap.png", 50, 50, true, true);
		pointSelected = new Image("media/tapSelected.png", 50, 50, true, true);
		...
		drawHands = new AnimationTimer() {
			...
			public void handle(long now) {
				...
				if (!exitImage.getBoundsInParent().contains(
						rightHand.getBoundsInParent())
						&& !addSquareBlockImage.getBoundsInParent().contains(
								rightHand.getBoundsInParent())
						&& !addRectBlockImage.getBoundsInParent().contains(
								rightHand.getBoundsInParent())) {
				...
				if (blocks.getChildren().size() < 25) {
					...
					if (addSquareBlockImage.getBoundsInParent().contains(
							rightHand.getBoundsInParent())) {
					...
					if (addRectBlockImage.getBoundsInParent().contains(
							rightHand.getBoundsInParent())) {
		...
		exit = new Timeline(new KeyFrame(Duration.seconds(.5), ae -> {
		...
		addRectBlock = new Timeline(new KeyFrame(Duration.seconds(.5), ae -> {
			rightHand.setImage(rightHandFull);
			addRectBlock();
			countPing.stop();
			countPing.play();
		...
		WorldModel.addWall(8, .5f, 16.0f, 1.0f); // not a wall actually the
													// ground
		...
		Stop[] stops = new Stop[] { new Stop(0, new Color(1, 1, 1, 0.0)),
		...
		KeyFrame frame = new KeyFrame(duration, ae -> {
			// Create time step. Set Iteration count 8 for velocity and 3 for
			// positions
				...
				for (int i = 0; i < arrayBlocks.size(); i++) {
					...
					current.node.setLayoutX(WorldModel
							.fromJPosXToSPosX((float) xpos)
							- current.getWidth() * 50);
					current.node.setLayoutY(WorldModel
							.fromJPosYToSPosY((float) ypos)
							- current.getHeight() * 50);
		...
		buttons.getChildren().addAll(exitImage, addSquareBlockImage,
				addRectBlockImage);
	...
	public void onFrame(Controller controller) {
		...
		if (extended.count() == 1) {
			pointPosX = Util.leapXtoPanelX(extended.get(0)
					.stabilizedTipPosition().getX());
			pointPosY = Util.leapYToPanelY(extended.get(0)
					.stabilizedTipPosition().getY());

			if (pointPosX > 325 && pointPosX < 1325 && pointPosY > 75
					&& pointPosY < 825) {
				// USER IS INSIDE BOUNDS
				for (int i = 0; i < arrayBlocks.size(); i++) {
					Block current = arrayBlocks.get(i);
					if (current.node.getBoundsInParent().contains(
							point.getBoundsInParent())) {
						selected = current;
						break;
					} else {
						selected = null;
				...
				if (selected != null) {
					...
					body.setTransform(
							new Vec2(
									(float) (pointPosX - 25) / 100,
									(float) ((WorldModel.HEIGHT - pointPosY + 25) / 100)),
							body.getAngle());
			...
			for (int i = 0; i < hands.count(); i++) {
				...
				if (hands.get(i).isRight()) {
					...
					left = hands.get(i);
					leftHandPosX = Util.leapXtoPanelX(left
							.stabilizedPalmPosition().getX());
					leftHandPosY = Util.leapYToPanelY(left
							.stabilizedPalmPosition().getY());
				}
			}
		}
